...
int main(int argc, char ** argv) {
    ...
    if (filename_photopeak_output == "" && filename_output == "") {
        cerr << "No output filenames specified" << endl;
        return(-5);
    }

    // If the output filename was specified, write out the photopeaks for free
    // even if the filename was not specified.
    ...
    // TODO: If the root output file is specified, write out the histograms

    // Write out to the photopeak file if specified
    if (filename_photopeak_output != "") {
        ofstream pp_output(filename_photopeak_output.c_str());
        for (int p = 0; p < config.panels_per_system; p++) {
            for (int c = 0; c < config.cartridges_per_panel; c++) {
                for (int f = 0; f < config.fins_per_cartridge; f++) {
                    for (int m = 0; m < config.modules_per_fin; m++) {
