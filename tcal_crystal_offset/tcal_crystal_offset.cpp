...
void usage() {
    cout << "tcal_crystal_offset [-vh] -c [config] -f [filename] -f ...\n"
         ...
         << "  -co [name]: optional output file of crystal offsets\n"
...
int main(int argc, char ** argv) {
    ...
    string filename_crystal_offset_output;
    ...
    for (int ix = 1; ix < (argc - 1); ix++) {
        ...
        if (argument == "-co") {
            filename_crystal_offset_output = following_argument;
        }
        if (argument == "-l") {
    ...
    if (verbose) {
        ...
        cout << "filename_crystal_offset_output: "
             << filename_crystal_offset_output << endl;
    ...
    vector<vector<vector<vector<vector<vector<float> > > > > > crystal_time_offset;
    vector<vector<vector<vector<vector<vector<TH1F*> > > > > > crystal_dtf;
    config.resizeArrayPCFMAX<TH1F*>(crystal_dtf, 0);
    config.resizeArrayPCFMAX<float>(crystal_time_offset, 0);
                    ...
                    for (int a = 0; a < config.apds_per_module; a++) {
                        for (int x = 0; x < config.crystals_per_apd; x++) {
                            ...
                            sprintf(namestring,
                                    ...
                                    p, c, f, m, a, x);
                            sprintf(titlestring,
                                    "Timing Spectra P%dC%dF%dM%dA%dX%d",
                                    p, c, f, m, a, x);
                            crystal_dtf[p][c][f][m][a][x] = new TH1F(
    ...
    for (int iter_no = 0; iter_no < no_iterations; iter_no++) {
            ...
            for (int c = 0; c < config.cartridges_per_panel; c++) {
                for (int f = 0; f < config.fins_per_cartridge; f++) {
                    for (int m = 0; m < config.modules_per_fin; m++) {
                        for (int a = 0; a < config.apds_per_module; a++) {
                            for (int x = 0; x < config.crystals_per_apd; x++) {
                                TH1F * h = crystal_dtf[p][c][f][m][a][x];
                                h->Reset();
                            }
                        }
                    }
        ...
        for (size_t ii = 0; ii < coinc_data.size(); ii++) {
            ...
            TH1F * crystal0_hist =
                    crystal_dtf[0][event.cartridge0][event.fin0][event.module0]
                               [event.apd0][event.crystal0];
            TH1F * crystal1_hist =
                    crystal_dtf[1][event.cartridge1][event.fin1][event.module1]
                               [event.apd1][event.crystal1];

            crystal0_hist->Fill(event.dtf);
            crystal1_hist->Fill(event.dtf);
        }
            ...
            for (int c = 0; c < config.cartridges_per_panel; c++) {
                for (int f = 0; f < config.fins_per_cartridge; f++) {
                    for (int m = 0; m < config.modules_per_fin; m++) {
                        for (int a = 0; a < config.apds_per_module; a++) {
                            for (int x = 0; x < config.crystals_per_apd; x++) {
                                crystal_time_offset[p][c][f][m][a][x] = alpha *
                                        calcCrystalOffset(
                                            crystal_dtf[p][c][f][m][a][x],
                                ...
                                if (p == 1) {
                                    crystal_time_offset[p][c][f][m][a][x] *= -1;
                                }
                            }
                        }
                    }
                    ...
                    for (int m = 0; m < config.modules_per_fin; m++) {
                        for (int a = 0; a < config.apds_per_module; a++) {
                            for (int x = 0; x < config.crystals_per_apd; x++) {
                                ...
                                x_cal.time_offset += crystal_time_offset[p][c][f][m][a][x];
                            }
                        }
                    }
                }
    ...
    if (filename_crystal_offset_output != "") {
        if (verbose) {
            cout << "Writing offsets: " << filename_crystal_offset_output << endl;
        }
        // Write out to the photopeak file
        ofstream crystal_output(filename_crystal_offset_output.c_str());
        for (int p = 0; p < config.panels_per_system; p++) {
            for (int c = 0; c < config.cartridges_per_panel; c++) {
                for (int f = 0; f < config.fins_per_cartridge; f++) {
                    for (int m = 0; m < config.modules_per_fin; m++) {
                        for (int a = 0; a < config.apds_per_module; a++) {
                            for (int x = 0; x < config.crystals_per_apd; x++) {
                                crystal_output << std::fixed << std::setprecision(1)
                                               << crystal_time_offset[p][c][f][m][a][x]
                                               << "\n";
                            }
                        }
                    }
