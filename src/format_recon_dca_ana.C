                    ...
                    if ( (data->crystal1>=0 ) && (data->crystal1<64 )) {
                        if ( (data->crystal2>=0 ) && (data->crystal2<64 )) {
                            ...
                            */
                            ...
                            // [x,y,z]10 is the vector from the source to one end of the LOR
                            // call it 'a'
                            x10 = x1- x0[sourcepos];
                            y10 = y1- y0[sourcepos];
                            z10 = z1- z0[sourcepos];

                            // [x,y,z]21 is the vector of the LOR
                            // call it 'b'
                            x21 = x2 - x1;
                            y21 = y2 - y1;
                            z21 = z2 - z1;

                            // x10_2 is the magnitude squared of a
                            x10_2 = x10*x10 + y10*y10 + z10*z10;
                            // x21_2 is the magnitude squared of b
                            x21_2 = x21*x21 + y21*y21 + z21*z21;
                            // x10x21 is a dot b
                            x10x21 = ( x10*x21 + y10*y21 + z10*z21);

                            // c = Projection of a onto b
                            Double_t c_x = x21 * x10x21 / x21_2;
                            Double_t c_y = y21 * x10x21 / x21_2;
                            Double_t c_z = z21 * x10x21 / x21_2;

                            // d = Projection of a onto DCA
                            Double_t d_x = x10 - c_x;
                            Double_t d_y = y10 - c_y;
                            Double_t d_z = z10 - c_z;

                            if (DCA_DIRECTION == 'x') {
                                d = TMath::Abs(d_x);
                            } else if (DCA_DIRECTION == 'y') {
                                d = TMath::Abs(d_y);
                            } else if (DCA_DIRECTION == 'z') {
                                d = TMath::Abs(d_z);
                            } else {
                                d = TMath::Sqrt(d_x*d_x + d_y*d_y + d_z*d_z);
                            }
                            //	 cout << d << endl;

                            // sqrt(||a||^2*||b||^2 - (a dot b)^2) / sqrt( ||b||^2)
                            //d = TMath::Sqrt(( x10_2*x21_2 - x10x21*x10x21) / ( x21_2 )) ;
