...
Int_t Sel_GetEhis::WriteHists(TFile *rfile)
{
    ...
    for (Int_t c=0; c<CARTRIDGES_PER_PANEL; c++) {
        for (Int_t f=0; f<FINS_PER_CARTRIDGE; f++) {
            ...
            if (!(f_subdir)) {
                f_subdir = rfile->mkdir(tmpstring);
            }
            if (verbose) {
                f_subdir->cd();
                cout << "F" << f << " : "  << endl;
            ...
            for (Int_t i=0; i<MODULES_PER_FIN; i++) {
                ...
                if (!(m_subdir)) {
                    m_subdir = f_subdir->mkdir(tmpstring);
                }
                m_subdir->cd();
                if (verbose) {
                    cout << "F" << f << "M" << i << " : " << endl;
                ...
                for (Int_t j=0; j<APDS_PER_MODULE; j++) {
                    ...
                    if (!(a_subdir)) {
                        a_subdir = m_subdir->mkdir(tmpstring);
                    }
                    a_subdir->cd();
                    if(verbose) {
                        cout << "F" << f << "M" << i << "A" << j << " : " << endl;
                        gDirectory->pwd();
                    }
                    // place to fit these histograms -- FIXME
                    if ( FitApdEhis(c,f,i,j) ) {
                        cout << " Something Wrong. Exiting function WriteHists.\n return -1;" << endl;
                    }
                    sprintf(tmpstring,"Spatial");
                    s_subdir = (TDirectory *) a_subdir->Get(tmpstring);
                    if (!(s_subdir)) {
                        s_subdir = a_subdir->mkdir(tmpstring);
                    }
                    s_subdir->cd();
                    if (verbose) {
...
Int_t Sel_GetEhis::FitApdEhis(Int_t cc, Int_t f, Int_t i, Int_t j)
{
    ...
    if (!fCrysCal) {
        cout << " Warning :: fCrysCal not defined. Please do : " << endl ;
    ...
    if (!fPPeaks) {
        cout << " Warning :: fPPeaks not defined. Please check. " << endl ;
        cout << " exiting. " << endl;
        return -1;
    }
    ...
    if (fCrysCal->validpeaks[cc][f][i][j]) {
        ...
        for (Int_t k=0; k<XTALS_PER_APD; k++) {
            ...
            if (TMath::Abs(fCrysCal->GainSpat[cc][f][i][j][k]/Emean -1 ) > 0.3 )          {
                if (verbose) {
                    cout << " BAD FIT to histogram ["<<c<<"]["<<f<<"]["<<i<<"]["<<j<<"]["<<k<<"]" << endl;
...
Int_t Sel_GetEhis::GenPlots(Int_t cc, Int_t firstfin, Int_t lastfin)
{
    ...
    if (lastfin > FINS_PER_CARTRIDGE ) {
        lastfin = FINS_PER_CARTRIDGE;
    }
    if (( cc > CARTRIDGES_PER_PANEL ) || (cc < 0 )) {
        cout << " Error in GenPlots. Please specify correct cartridge " << endl;
    }
