...
//               TH1F *hi = (TH1F *) f->Get("C0F6/M8A0/xhist[6][8][0][3]")
	    sprintf(tmpstring, "C%dF%d/M%dA%d/xhist[%d][%d][%d][%d]", cc,ff,mm,aa,ff,mm,aa,k);
                ...
                if (!xfits[k]) {cout << " Couldn't extract xfits["<<k<<"]" << endl;  
...
/*
	for(Int_t i = 0; i < 64; i++) {
		sprintf(tmpstring, "ypos[%d];1", i);
                else	ypos[i] = (TH1F *) f->Get(tmpstring);
	}
*/


	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
//	TGraph		*peaks = (TGraph *) f->Get("peaks_sorted");
//	Double_t	*xv = (Double_t *) peaks->GetX();
//	Double_t	*yv = (Double_t *) peaks->GetY();

//	cout <<" << xv[0] = " << xv[0] <<" .. yv[0] = " << yv[0] <<endl;

//	TNtuple *dat = (TNtuple *) f->Get("datantuple;1");
//	dat->SetName("dat");
...
/*
	TVector *peVals = (TVector *) f->Get("TVectorT<float>");
	Float_t E_low, E_up;
	Float_t E_com_low, E_com_up;
	//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

	E_low = (Float_t) (*peVals) (3);
	E_up = (Float_t) (*peVals) (4);
	E_com_low = (Float_t) (*peVals) (5);
	E_com_up = (Float_t) (*peVals) (6);


        if (verbose){
	    cout << "   ========================================================== " << endl;
	    cout << "   =   " << E_low << " < E  < " << E_up << "                          = " << endl;
	    cout << "   =   " << E_com_low << " < Ec < " << E_com_up << "                  = " << endl;
	    cout << "   ========================================================== " << endl;
	}
...
//	fitall(hists, ftmp, &vals[0], pixvals, E_low, E_up, c1, verbose, SPATSATURATION);
	

/*
	
	for(Int_t i = 0; i < PEAKS; i++) {
		sprintf(tmpstring, "ffun[%d]", i);
		ffun[i] = (TF1 *) ftmp[i]->Clone();
	}
...
*/
	...
	if (verbose)        cout << " Just finished the Spatials,  " <<endl;
	/*
	 * gROOT->ProcessLine(".L apd_draw.C");
	 */
/*

 // FIXME  Do we really need draw_results ??
	draw_results(&vals[0], 1, c1, dat, back2,pixvals[28][0]*1.1);
		sprintf(curoutfile, "%s_E_fits.spat.sum.ps", basestring);
		c1->Print(curoutfile);

  cout << " You should see this statement ! " << endl;

  	sprintf(curoutfile, "./%s_E_fits.spat.sum.png", basestring);

       cout << " curoutfile = " << curoutfile << endl;
...
//        cout << " did we get here  ?? " <<endl;
	...
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
	/*
	 * c1->Print("042109_409-1-17-A_Efits.spat.sum.png");
	 */
//	TH1F	*mean_spat = (TH1F *) mean->Clone("mean_spat");
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/

//	mean_spat->SetName("mean_spat");

	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
//	TH1F	*fwhmE_spat = (TH1F *) fwhmE->Clone("fwhmE_spat");
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/

//	fwhmE_spat->SetName("fwhmE_spat");

	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
//	TH1F	*fwhmE3_spat = (TH1F *) fwhmE3->Clone("fwhmE3_spat");
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/

//	fwhmE3_spat->SetName("fwhmE3_spat");
...
//        if (E_com_up> COMSATURATION ) { E_com_up=COMSATURATION-50;  E_com_up - = .3 * (E_com_up - E_com_low);}

/*
        if (verbose) cout << " Fitting 64 commons between " << E_com_low << " and "  << E_com_up << endl;

	fitall(hists_com, ftmp, &com_vals[0], com_pixvals, E_com_low,E_com_up, c1, verbose, COMSATURATION);

	for(Int_t i = 0; i < PEAKS; i++) {
		sprintf(tmpstring, "ffun_com[%d]", i);
		ffun_com[i] = (TF1 *) ftmp[i]->Clone();
	}
*/

// cout <<" << xv[0] = " << xv[0] <<" .. yv[0] = " << yv[0] <<endl;
/*
	sprintf(curoutfile, "%s.fitval.com.txt", basestring);
	myfile.open(curoutfile);

	myfile <<
		"## X Y position pixel, MEAN, MEAN_ERR, FWHM, FWHM_ERR, CHi2PERNDF, MEAN_2ND_GAUS, MEAN_2ND_GAUS_E, FWHM_2ND_GAUS, FWHM_2ND_GAUS_E, ERES, ERES_ERR " <<
		endl;
	for(j = 0; j < PEAKS; j++) {
		myfile << xv[j] << "  " << yv[j] << "  ";
		for(k = 0; k < 9; k++) {
			myfile << com_pixvals[j][k] << "  ";
		}

		if(com_pixvals[j][2]) {
			myfile << 100 * com_pixvals[j][2] / com_pixvals[j][0] << "  ";
			myfile <<
				100 *
				errorprop_divide(com_pixvals[j][2], com_pixvals[j][3], com_pixvals[j][0], com_pixvals[j][1]) <<
				"  ";
		}
		else {
			myfile << "-99" << "  " << "99" << endl;
...
/*
	draw_results(&com_vals[0], 2, c1, dat, back2,com_pixvals[28][0]*1.1);

	sprintf(curoutfile, "%s_E_fits.com.sum.ps", basestring);
	c1->Print(curoutfile);

	sprintf(curoutfile, "%s_E_fits.com.sum.png", basestring);
	c1->Print(curoutfile);
*/
	/*
	 * c1->Print("042109_409-1-17-A_Efits.com.sum.ps");
	 * c1->Print("042109_409-1-17-A_Efits.com.sum.png");
	 */

/*
	sprintf(curoutfile, "%s.fitval.all.txt", basestring);
	myfile.open(curoutfile);
	myfile <<
		"## X Y position pixel, MEAN, MEAN_ERR, FWHM, FWHM_ERR, CHi2PERNDF, MEAN_2ND_GAUS, MEAN_2ND_GAUS_E, FWHM_2ND_GAUS, FWHM_2ND_GAUS_E, ERES, ERES_ERR, MEAN_COM, MEAN_ERR_COM, FWHM_COM, FWHM_ERR_COM, CHi2PERNDF_COM, MEAN_2ND_GAUS_COM, MEAN_2ND_GAUS_E_COM, FWHM_2ND_GAUS_COM, FWHM_2ND_GAUS_E_COM, ERES_COM, ERES_ERR_COM" <<
		endl;
	for(j = 0; j < PEAKS; j++) {
		myfile << xv[j] << "  " << yv[j] << "  ";
		for(k = 0; k < 9; k++) {
		...
		if(pixvals[j][2]) {
			myfile << 100 * pixvals[j][0] / pixvals[j][2] << "  ";
			myfile << 100 * pixvals[j][0] / pixvals[j][2] << "  ";
		}
		else {
			myfile << "-99" << "  " << "99" << endl;
		...
		if(com_pixvals[j][2]) {
			myfile << 100 * com_pixvals[j][0] / com_pixvals[j][2] << "  ";
			myfile << 100 * com_pixvals[j][0] / com_pixvals[j][2] << "  ";
		}
		else {
			myfile << "-88" << "  " << "88" << endl;
...
// cout << " << xpeak[0] = " << xv[0] <<" .. ypeak[0] = " << yv[0] <<endl;
	...
	if(verbose) {
        ...
        TH1F *espec_glob_spat, *espec_glob_com;
       
//        Int_t numBins = BINS_PER_SPAT_PHOTOPEAK * ((1.4 * E_up) / (E_up - E_low));
...
/*
        Float_t Com_low,Com_up;
        Com_low = (Float_t) (*peVals) (5);
	Com_up = (Float_t) (*peVals) (6);

	numBins = BINS_PER_COM_PHOTOPEAK * ((1.4 * Com_up) / (Com_up - Com_low));			
*/

        HistString.Form("C%dF%d/GlobEhist_com_C%dF%dM%dA%d",cc,ff,cc,ff,mm,aa);
        espec_glob_com = (TH1F *) globrootfile->Get(HistString.Data());

//	 espec_glob_com = new TH1F("espec_glob_com","Global Energy Spectrum Common", numBins, 0, 1.4 * Com_up);


	 //	if(tacana) {
//	 peaktovalley_refill(pixvals, com_pixvals, dat, yv, xv, xpos, ypos, ANGER, (*peVals) (1), (*peVals) (2), tacs, espec_glob_spat, espec_glob_com, verbose,tacana);
		//	}
 ...
 for (Int_t j=0;j<PEAKS;j++){
  ...
  TF1 *glob_spat_fit = (TF1 *) espec_glob_spat->GetListOfFunctions()->FindObject("fitfunc");
         ...
         TF1 *glob_com_fit = (TF1 *) espec_glob_com->GetListOfFunctions()->FindObject("fitfunc");
...
/*	fitpos(xpos, verbose);
	if(verbose) {
		cout << " Positions fitted " << endl;
	}
*/

/*
	  TF1 *fpos1[64],*fpos2[64];
	  for (Int_t i=0;i<64; i++){ sprintf(tmpstring,"fpos1[%d]",i);
	  fpos1[i] = new TF1(tmpstring,"gaus");
	  sprintf(tmpstring,"fpos2[%d]",i);
	  fpos2[i] = new TF1(tmpstring,"gaus");
	  } 
          writ(xpos,fpos1,c1,"xpos.tmp",0,PSFILE);
	  writ(ypos,fpos2,c1,"ypos.tmp",0,PSFILE);
*/	

//	peaktovalley(xpos, c1);

	/*~~~~~~~~~~~~~~~~*/
	
//	writ(xpos,c1,"xpos_test.ps",verbose);
	 
	Double_t	*FOM[8];
	/*~~~~~~~~~~~~~~~~*/

//	FOM[0] = ptv_ana(xsums[0], c1, 0, 1, verbose);
//	sprintf(curoutfile, "%s_FOM.ps(", basestring);
...
//	FOM[7] = ptv_ana(xsums[7], c1, 7, 1, verbose);
//	sprintf(curoutfile, "%s_FOM.ps)", basestring);
//	c1->Print(curoutfile);


Double_t FOM_CTR_AV ;
 if (FOM[3][2] && FOM[4][2]) FOM_CTR_AV = ( FOM[3][0]/FOM[3][2] + FOM[4][0]/FOM[4][2]);
 else FOM_CTR_AV = 0;
 FOM_CTR_AV/=2;

 Double_t FOM_TOP_AV ;
   if (FOM[0][2] && FOM[7][2]) FOM_TOP_AV= ( FOM[0][0]/FOM[0][2] + FOM[7][0]/FOM[7][2]);
 else FOM_TOP_AV = 0;
 FOM_TOP_AV/=2;

 Double_t EDGE_FOM_CTR_AV ;
 if (FOM[3][6] && FOM[4][6]) EDGE_FOM_CTR_AV = ( FOM[3][4]/FOM[3][6] + FOM[4][4]/FOM[4][6]);
 else EDGE_FOM_CTR_AV = 0;
 EDGE_FOM_CTR_AV/=2;

 Double_t EDGE_FOM_TOP_AV ;
   if (FOM[0][2] && FOM[7][2]) EDGE_FOM_TOP_AV= ( FOM[0][4]/FOM[0][6] + FOM[7][4]/FOM[7][6]);
...
// New Code HERE (from Email) 
 ...
 sprintf(curoutfile,"%s_%s_FOM.txt",basestring,modulestringcfma);
	...
	/*
	 * c2->Divide(3,3,0.00001,0.00001,0);
	 */
...
//        cout << " MAX ::  " << SpatMeans[spatmap[63]] << " MIN : " << SpatMeans[spatmap[0]] << endl;
...
//        cout << "SpatEres->GetMean() :: " << TMath::Mean(64,SpatEres->GetY()) << endl;
...
Int_t SCOREMASK=0;
 ...
 // if (gain_range < 25 ) {
  if (gain_range < 35 ) {
   ...
   if (gain_range < 50 )  {scorecard_gain->SetFillColor(kOrange);SCOREMASK |= GAIN_SPAT_INT;}
   else scorecard_gain->SetFillColor(kRed);
 }
...
TPaveText *scorecard_gain_com = new TPaveText(.83,.255,.97,.295,"NDC");
 ...
 if (gain_range_com < 50 ) {scorecard_gain_com->SetFillColor(kOrange); SCOREMASK |= GAIN_COM_INT;}
   else scorecard_gain_com->SetFillColor(kRed);
 }
 ...
 if (score & 0x1) {SCOREMASK |= FOM_TOP;}
 else {if (score & 0x2) SCOREMASK |= FOM_TOP_INT;}
...
TPaveText *scorecard_FOM_edge_center = new TPaveText(.83,.2,.895,.24,"NDC");
 ...
 if (score & 0x1) {SCOREMASK |= EDGE_FOM_CTR;}
 else {if (score & 0x2) SCOREMASK |= EDGE_FOM_CTR_INT;}
...
TPaveText *scorecard_FOM_edge_top = new TPaveText(.905,.2,.97,.24,"NDC");
 ...
 // scorecard_FOM_edge_top->SetFillColor(GiveCol( (Float_t) 1/EDGE_FOM_TOP_AV,1/3.,1/3.,&score));
 scorecard_FOM_edge_top->SetFillColor(GiveCol( (Float_t) 1/EDGE_FOM_TOP_AV,1/3.,1/2.5,&score));
 if (score & 0x1) {SCOREMASK |= EDGE_FOM_TOP;}
 else {if (score & 0x2) SCOREMASK |= EDGE_FOM_TOP_INT;}
...
TPaveText *scorecard_FOM_center = new TPaveText(.755,.2,.82,.24,"NDC");
 ...
 if (score & 0x1){ SCOREMASK |= FOM_CTR;}
 else {if (score & 0x2) SCOREMASK |= FOM_CTR_INT;}
 ...
 if (score & 0x1) {SCOREMASK |= RATIO;}
 else {if (score & 0x2) SCOREMASK |= RATIO_INT;}
...
TPaveText *scorecard_flood = new TPaveText(.91,.035,.97,.075,"NDC");
 ...
 if (score & 0x1) {SCOREMASK |= FLOOD_UNI;}
 else {if (score & 0x2) SCOREMASK |= FLOOD_UNI_INT;}
 ...
 if (score & 0x1) {SCOREMASK |= ERES_SPAT;}
 else {if (score & 0x2) SCOREMASK |= ERES_SPAT_INT;}
 scorecard_eres->Draw();
 ...
 if (score & 0x1) {SCOREMASK |= ERES_COM;}
 else {if (score & 0x2) SCOREMASK |= ERES_COM_INT;}
 ...
 if (score & 0x1) {SCOREMASK |= ERES_SPAT_RMS;}
 else {if (score & 0x2) SCOREMASK |= ERES_SPAT_RMS_INT;}
 ...
 if (score & 0x1) {SCOREMASK |= ERES_COM_RMS;}
 else {if (score & 0x2) SCOREMASK |= ERES_COM_RMS_INT;}
 ...
 /*
	...
	//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	TPaveText	*result = new TPaveText(.7, .075, .95, .20, "NDC");
	//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

	sprintf(tmpstring, "E_res COM  = %.1f%s RMS = %.1f%s", 100 * fwhmE3->GetMean(), "%", 100 * fwhmE3->GetRMS(), "%");
	result->AddText(tmpstring);
	sprintf
	(
		tmpstring,
		"E_res SPAT = %.1f%s RMS = %.1f%s",
		100 * fwhmE3_spat->GetMean(),
		"%",
		100 * fwhmE3_spat->GetRMS(),
		"%"
	);
	result->AddText(tmpstring);
	result->Draw();
...
/*
	...
	globfile << 100 * vals[2] / vals[0] << "  ";
	globfile << 100 * TMath::Sqrt( TMath::Power((vals[3] / vals[0]), 2) + TMath::Power((vals[2] * vals[1] / (vals[0] * vals[0])), 2) ) << "  ";

	for(k = 0; k < 4; k++) {
		globfile << com_vals[k] << "  ";
	}

	globfile << 100 * com_vals[2] / com_vals[0] << "  ";
	globfile << 100 * TMath::Sqrt
		(
			TMath::Power((com_vals[3] / com_vals[0]), 2) + TMath::Power
				(
					(com_vals[2] * com_vals[1] / (com_vals[0] * com_vals[0])),
					2
				)
		) << "  ";
*/
...
<< "  " ;  }
	...
	globfile << 100 * 2.35 * TMath::Sqrt( TMath::Power((glob_spat_fit->GetParError(5) / glob_spat_fit->GetParameter(4)), 2) + TMath::Power((glob_spat_fit->GetParameter(5) * glob_spat_fit->GetParError(4) / (glob_spat_fit->GetParameter(4)*glob_spat_fit->GetParameter(4))), 2) ) << "  ";
         } 
         else {globfile <<  "0   0" ;}
  ...
  for (Int_t k=0;k<6;k++){
	...
	globfile << 100 * 2.35 * TMath::Sqrt( TMath::Power((glob_com_fit->GetParError(5) / glob_com_fit->GetParameter(4)), 2) + TMath::Power((glob_com_fit->GetParameter(5) * glob_com_fit->GetParError(4) / (glob_com_fit->GetParameter(4)*glob_com_fit->GetParameter(4))), 2) ) << "  ";
         } 
         else {globfile <<  "0   0" ;}
...
Int_t GiveCol(Float_t aa, Float_t a1, Float_t a2, Int_t* RESULT){
   ...
   *RESULT=0;
   if ( aa < a1 ) { (*RESULT) |= 0x1; return kGreen+1;}
  else {
    if ( aa < a2) { (*RESULT) |= 0x2; return kOrange;}
    else { return kRed;}
   }
 }
...
Double_t *ptv_ana(TH1F *hist, TF1 *posgausfits[64], TCanvas *c1, Int_t nr, Int_t x,Int_t verbose){
  ...
  for (i=p;i<(8-p);i++){
    if (x) {index = 8*nr+i;next=8*nr+i+1;}
    else {index = 8*i+nr;next=8*(i+1)+nr;}
    if (verbose) { cout << " index = " << index << " next =" << next << endl;}
...
// Analyze the edge
 ...
 for (i=0;i<2;i++){
  if (x) { index = 8*nr+i*6;next=8*nr+i*6+1;}
    ...
    if (verbose){
      cout << " i = " << i << " :: " << posgausfits[next]->GetParameter(1)-posgausfits[index]->GetParameter(1) << " edge_mean variance :: " << edge_mean_variance <<  "  edge mean :: " << edge_dist << endl;}
