...
#define UNUSEDCHANNELOFFSET 0
                ...
                for (int iii=0;iii<4;iii++ ) {
                    if ( trigCode & ( 0x1 << iii )) {
                        ...
                        if (uvcalc) {
                            // to calculate circle centers we need to make sure
                            // the module triggered, so we use a high threshold

                            // if (verbose) { 
                            //     cout << " Calculating UV " << " entries :: " << rawdata->GetEntries() << endl;
                            // }
                            ...
                            //                 << rawevent.com1h 
                            //                 << " pedestal: " 
                            //                 << pedestals[chip][module][5];
                            //         cout << " Com1h-ped: " 
                            //                 << rawevent.com1h - pedestals[chip][module][5] 
                            //                 << " valid: ";
                            //         cout <<  ((rawevent.com1h - pedestals[chip][module][5]) < uvthreshold) 
                            //                 << endl ;
                            //    }
                            //}
                            if (  ( rawevent.com1h - pedestals[chipId][module][5] ) < uvthreshold ) {
                                uventries[chipId][module][0]++;
                                uu_c(chipId+module*RENACHIPS+0) += 
                                    (Float_t)(rawevent.u1h-uu_c(chipId+module*RENACHIPS+0))/uventries[chipId][module][0]; 
                                vv_c(chipId+module*RENACHIPS+0) +=
                                    (Float_t)(rawevent.v1h-vv_c(chipId+module*RENACHIPS+0))/uventries[chipId][module][0]; 
                            }
                            if (( rawevent.com2h - pedestals[chipId][module][7] ) < uvthreshold ) {
                                uventries[chipId][module][1]++;
                                uu_c(chipId+RENACHIPS*module+1*MODULES*RENACHIPS) +=
                                    (Float_t)(rawevent.u2h-uu_c(chipId+module*RENACHIPS+1*MODULES*RENACHIPS))/uventries[chipId][module][1]; 
                                vv_c(chipId+RENACHIPS*module+1*MODULES*RENACHIPS) +=
                                    (Float_t)(rawevent.v2h-vv_c(chipId+module*RENACHIPS+1*MODULES*RENACHIPS))/uventries[chipId][module][1]; 
                            }
                        } // if uvcalc
    ...
    if (calcpedestal==1){
        ...
        for (int  ii = 0 ; ii< rawdata->GetEntries(); ii++ ){
            ...
            pedana( &ped_ana[chip][module][0][0], 
                    &ped_ana[chip][module][0][1],
                    ped_evts[chip][module],
                    rawevent.a );
            pedana( &ped_ana[chip][module][1][0],
                    &ped_ana[chip][module][1][1],
                    ped_evts[chip][module],
                    rawevent.b );
            pedana( &ped_ana[chip][module][2][0],
                    &ped_ana[chip][module][2][1],
                    ped_evts[chip][module],
                    rawevent.c );
            pedana( &ped_ana[chip][module][3][0],
                    &ped_ana[chip][module][3][1],
                    ped_evts[chip][module],
                    rawevent.d );
            pedana( &ped_ana[chip][module][4][0],
                    &ped_ana[chip][module][4][1],
                    ped_evts[chip][module],
                    rawevent.com1 );
            pedana( &ped_ana[chip][module][5][0],
                    &ped_ana[chip][module][5][1],
                    ped_evts[chip][module],
                    rawevent.com1h );
            pedana( &ped_ana[chip][module][6][0],
                    &ped_ana[chip][module][6][1],
                    ped_evts[chip][module],
                    rawevent.com2 );
            pedana( &ped_ana[chip][module][7][0],
                    &ped_ana[chip][module][7][1],
                    ped_evts[chip][module],
                    rawevent.com2h );
        }
        ...
        for (int j=0; j<RENACHIPS;j++){
            for (int jj=0;jj<MODULES;jj++){
                ...
                for (int jjj=0;jjj<8;jjj++){
                    ...
                    if ( ped_evts[j][jj] ) {
                        pedfile  << TMath::Sqrt(ped_ana[j][jj][jjj][1]/ped_evts[j][jj]) << " ";
                    } else {
                        pedfile << "0 ";
                    }
                }
        ...
        cout << "file:: " << filename  << " ped :: " << ped_ana[1][0][0][0] 
            << " RMS :: " << TMath::Sqrt(ped_ana[1][0][0][1]/ped_evts[1][0]);
